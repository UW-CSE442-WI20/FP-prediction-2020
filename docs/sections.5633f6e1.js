...
function scroller() {
  ...
  var sections = null; // array that will hold the y coordinate of each section that is scrolled through

  var sectionPositions = [];
  var currentIndex = -1; // y coordinate of
  ...
  function scroll(els) {
    sections = els; // when window is scrolled call position. When it is resized call resize.

    d3.select(window).on('scroll.scroller', position).on('resize.scroller', resize); // manually call resize initially to setup scroller.

    resize(); // hack to get position to be called once for the scroll position on load.
...
var scrollVis = function scrollVis() {
  ...
  var lastIndex = -1;
  var activeIndex = 0; // main svg used for visualization

  var svg = null; // d3 selection that will be used for displaying visualizations
  ...
  var chart = function chart(selection) {
    selection.each(function () {
      ...
      var path = d3.geoPath().projection(projection); // create svg

      svg = d3.select(this).selectAll('svg'); //      svg = d3.select(this).selectAll('svg').data([wordData]);

      var svgE = svg.enter().append('svg'); // @v4 use merge to combine enter and existing selection

      svg = svg.merge(svgE); // give svg a width and height

      svg.attr('width', width + margin.left + margin.right);
      svg.attr('height', height + margin.top + margin.bottom);
      svg.append('g'); // this group element will be used to contain all other elements.
  ...
  var setupVis = function setupVis(stateInfo, stateWinners, countyInfo, countyWinners, years) {
    d3.json("https://cdn.jsdelivr.net/npm/us-atlas@3/counties-10m.json").then(function (json) {
      ...
      for (var i = 0; i < years.length; i++) {
        ...
        var state_mapE = state_map.enter().append('path').classed(class_name, true);
        state_map = state_map.merge(state_mapE).attr("id", function (d) {
          return "c" + d.id;
        }).attr("class", function (d) {
          var temp = winner.find(function (x) {
            return x.id === d.id;
          });
          ...
          ;
        }).attr('d', path) //          .on("click", zoomToState)
  ...
  function result_2000() {
    g.selectAll('.2004map').transition().duration(0).attr('opacity', 0);
    g.selectAll('.2000map').transition().duration(500).attr('opacity', 1.0);
  }

  function result_2004() {
    g.selectAll('.2000map').transition().duration(0).attr('opacity', 0);
    g.selectAll('.2004map').transition().duration(500).attr('opacity', 1.0);
    g.selectAll('.2008map').transition().duration(0).attr('opacity', 0);
  }

  function result_2008() {
    g.selectAll('.2004map').transition().duration(0).attr('opacity', 0);
    g.selectAll('.2008map').transition().duration(500).attr('opacity', 1.0);
    g.selectAll('.2012map').transition().duration(0).attr('opacity', 0);
  }

  function result_2012() {
    g.selectAll('.2008map').transition().duration(0).attr('opacity', 0);
    g.selectAll('.2012map').transition().duration(500).attr('opacity', 1.0);
    g.selectAll('.2016map').transition().duration(0).attr('opacity', 0);
  }

  function result_2016() {
    g.selectAll('.2012map').transition().duration(0).attr('opacity', 0);
    g.selectAll('.2016map').transition().duration(500).attr('opacity', 1.0); //	g.selectAll('.2008map')
    //      .transition()
    //      .duration(0)
    //      .attr('opacity', 0);
  }
    ...
    d3.csv(state_csv).then(function (data) {
      for (var j = 0; j < years.length; j++) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          winner.push({
            id: temp_id,
            winner_name: data[i][ypar] + " " + data[i][yswi]
          });
        }
...
function display() {
  ...
  var plot = scrollVis();
  d3.select('#vis').call(plot); // setup scroll functionality

  var scroll = scroller().container(d3.select('#graphic')); // pass in .step selection as the steps

  scroll(d3.selectAll('.step')); // setup event handling
...
//# sourceMappingURL=https://uw-cse442-wi20.github.io/FP-prediction-2020/sections.5633f6e1.js.map